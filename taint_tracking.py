...
def is_tainted(var, state=None, taint_buf=TAINT_BUF):
    ...
    try:
        if type(var) == int:
            return False
        if taint_buf not in str(var.variables):
            return False
    except:
        print("error in is_tainted")
...
def is_or_points_to_tainted_data(x, state, size=64):
    if is_tainted(x, state):
        return True
    if type(x) == int or x.concrete==True:
        return is_tainted(state.memory.load(x, size=size), state)
    else:
        return False
...
def apply_taint(state, addr, taint_id='', bits=PAGE_SIZE, var=None, inspect=False, disable_actions=True):
    """
    ...
    state.memory.store(addr, var, inspect=inspect, disable_actions=disable_actions)
...
class TaintTracker(ExplorationTechnique):
    ...
    def __init__(self,
        ...
        self._N = n_iter_loop
        self._stop = Event()
        self._avoid_addrs = avoid_addrs if avoid_addrs else []
        self.optimistic_solve = optimistic_solve
        if rodata_range is not None:
            self._data_start = rodata_range
            self._data_end = rodata_range
    ...
    def default_concretization_strategy(state, cnt):
        ...
        if len(concs) <= 20:
            # switch number less than 20
            return concs
        return rd.choice(concs)
    ...
    def _addr_concrete_after(self, state):
        ...
        try:
            concrete_res = self._get_target_concretization(addr_expr, state)
            if type(concrete_res) is list:
                state.inspect.address_concretization_result = concrete_res
            else:
                state.inspect.address_concretization_result = [concrete_res]
        except:
            return
            ...
            if cnt.symbolic:
                ...
                if key_cnt in self._concretizations.keys():
                    ...
                    if state.solver.solution(cnt, conc):
                        ...
                        if type(conc) is not list or len(conc) == 1:
                            state.add_constraints(cnt == conc)
                        continue
                conc = self._concretization_strategy(state, cnt)
                self._concretizations[key_cnt] = conc
                if type(conc) is not list or len(conc) == 1 or all([(v > self._data_start and v < self._data_end) for v in conc]):
                    state.add_constraints(cnt == conc)
                else:
                    print(conc)
        if satisfiable and not state.solver.satisfiable():
            oldcon_cache_keys = []
            for con in state.solver.constraints:
                oldcon_cache_keys.append(con.cache_key)
            state.solver.reload_solver(state.solver.constraints)
            state.solver.constraints.sort(key=lambda x: oldcon_cache_keys.index(x.cache_key))
        val = state.solver.eval_upto(var, 20)
        if not val:
            return val
        if all([(v > self._data_start and v < self._data_end) for v in val]):
            print("val: ", val)
            return val
        return val[0]
    ...
    def _drop_constraints(self, state, optimistic_solve=False):
        if not optimistic_solve or not state.solver.constraints:
            state.solver._stored_solver.constraints = []
            state.solver.reload_solver()
            state.globals["unsat"] = True
            return
        ...
        try:
            ...
            if not self.optimistic_satisfiable(state, state.solver.constraints, last_constraint, oldcon_cache_keys):
                log.warning("unsat state %r optimistic solve fail!", state)
                state.solver._stored_solver.constraints = []
                state.solver.reload_solver()
        except Exception as e:
            log.warning("unsat optimisitic solve error: %r", e)
    ...
    def optimistic_satisfiable(self, state, constraints, target: ast.Bool, oldcon_cache_keys):
        ...
        for c in constraints:
            if c.variables & target.variables:
                relevant_constraints.append(c)
            else:
                irrelevant_constraints.append(c)
        state.solver.reload_solver(irrelevant_constraints)
    ...
    def _get_calling_convention_precise(self, state):
        ...
        try:
            cca = self.project.analyses.CallingConvention(f, analyze_callsites=True)
            if cca.cc:
                return cca.cc
        except Exception as e:
    ...
    def _get_calling_convention_fast(self, state):
        ...
        if state.arch.name in arg_order:
        ...
        assert state.arch.name == 'X86'
        try:
            caller_bl = self.project.factory.block(state.history.addr)
        except:
            raise
        sim_args = []
        ret = angr.calling_conventions.SimRegArg('eax', 32)
        stack_offset = 4
        for ins in caller_bl.capstone.insns[::-1]:
            if ins.mnemonic == 'push':
                push_reg = state.arch.get_register_by_name(ins.op_str)
                if push_reg is None:
                    wid_dscr = ins.op_str.split()[0]
                    if is_number(wid_dscr):
                        wid = 4
                    else:
                        assert wid_dscr in ('byte', 'word', 'dword', 'qword')
                        wid = 2 ** ('byte', 'word', 'dword', 'qword').index(wid_dscr)

                else:
                    wid = push_reg.size

                sim_args.append(angr.calling_conventions.SimStackArg(stack_offset, wid))
                stack_offset += wid
        if not sim_args:
            for ins in caller_bl.capstone.insns:
                if ins.mnemonic == 'mov':
                    wid_dscr = ins.op_str.split(', ')[0]
                    if '[esp' in wid_dscr:
                        sim_args.append(angr.calling_conventions.SimStackArg(stack_offset, 4))
                        stack_offset += 4
    ...
    def setup(self, simgr):
        # Access the binary's sections and segments
        for section in self.project.loader.main_object.sections:
            if section.name == '.rodata':
                # Print the start and end addresses of the .data segment
                self._data_start = section.min_addr
                self._data_end = section.max_addr
                # print(f".rodata segment start address: {hex(self._data_start)}")
                # print(f".rodata segment end address: {hex(self._data_end)}")
                break
    ...
    def step(self, simgr, *kargs, **kwargs):
        ...
        for s in simgr.active:
            state_copy(s)
                # s.globals[DEREFS] = copy.deepcopy(s.globals[DEREFS])
        ...
        for state in simgr.active:
            if not state.satisfiable():
                self._drop_constraints(state, self.optimistic_solve)
        ...
        for state in list(simgr.active):
            try:
                if state.addr in self._avoid_addrs:
        ...
        for s in list(simgr.active):
            ...
            if s.history.jumpkind == "Ijk_Call" or (s.history.jumpkind == "Ijk_Boring" and s.addr in self._function_blacklist):
                # when call a no-ret function, it might use `jmp func`
            ...
            if s.addr in (BOGUS_RETURN, BOGUS_RETURN_64) or s.globals[CURRENT_IFL] > self._interfunction_level + 3:
                log.debug("Returned out of the original function!")
                self.ret_states.append(s)
                simgr.active.remove(s)
                # simgr.deadended.append(s)
