...
class Utils:
    ...
    def arg_to_offset_other(self, arg_num, arch_name):
        if 'ARM' in arch_name:
            args_name = ['r0', 'r1', 'r2', 'r3']
        elif arch_name == 'MIPS32':
            args_name = ['a0', 'a1', 'a2', 'a3']
        elif arch_name == 'AMD64':
            args_name = ['rdi', 'rsi', 'rdx', 'rcx']

        else:
            raise NotImplementedError("arch not realize!")
        return self.arch.registers[args_name[arg_num - 1]][0]

    def arg_to_reg(self, arg_num, sz=8):
        return self.vex_to_name(self.arg_to_offset(arg_num), sz)

    def arg_to_reg_other(self, arg_num, arch_name, sz=8):
        return self.vex_to_name(self.arg_to_offset_other(arg_num, arch_name), sz)
